...
class EmojiRepositoryImpl implements EmojiRepository {
  ...
  EmojiRepositoryImpl({EmojiData? emojiData})
  ...
  Future<List<Emoji>> getEmojisByCategory(EmojiCategory category) async {
    ...
    if (category == EmojiCategory.recent) return getRecentEmojis();
  ...
  Future<List<Emoji>> getRecentEmojis({int limit = 30}) async {
    await _ensureInitialized();
    final recentCodepoints =
        _recentEmojis.take(limit).map((r) => r.codepoint).toList();
    final emojis = <Emoji>[];
    for (final codepoint in recentCodepoints) {
      final emoji = _emojiData.getEmoji(codepoint);
  ...
  Future<void> recordEmojiUsage(String codepoint) async {
    await _ensureInitialized();
    final existingIndex =
        _recentEmojis.indexWhere((r) => r.codepoint == codepoint);
    if (existingIndex >= 0) {
